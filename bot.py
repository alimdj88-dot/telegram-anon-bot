...
def handle_messages(message):
    ...
    if message.text == "📢 طنین مدیریت" and uid == OWNER_ID:
    ...
    # --- مدیریت چت زنده و تاریخچه گزارشات ---
    ...
    if user.get("state") == "in_chat":
        ...
        if message.text == "✂️ قطع ارتباط":
            ...
            reasons = ["توهین و بی‌ادبی 🤬", "تبلیغات 📢", "نامناسب 🔞", "مزاحمت ❌"]
    ...
    if user["state"] == "reg_name":
        ...
        bot.send_message(uid, "✨ حالا بگو شوالیه‌ای یا بانو؟", reply_markup=btn); return
    ...
    if user["state"] == "writing_confession" and (message.text or message.photo):
        db["users"][uid].update({"state": "confirm_confession", "temp_msg": message.text if message.text else "[مدیا]"}); save_db(db)
        ...
        bot.send_message(uid, "📜 بفرستمش برای صاحب راز؟", reply_markup=btn); return
    ...
    if message.text == "🛰 شکار هم‌صحبت":
        ...
        bot.send_message(uid, "🔍 دنبال کی مد نظرته؟", reply_markup=btn)
...
def callbacks(call):
    ...
    if call.data.startswith("view_msg_"):
        ...
        try:
            ...
            db["users"][uid]["state"] = "main"; save_db(db)
            bot.edit_message_text("✅ پاسخ تو با موفقیت ارسال شد.", uid, call.message.id)
        ...
        btn = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🚫 بن (BAN)", callback_data=f"adm_ban_{partner}"), types.InlineKeyboardButton("✅ نادیده گرفتن", callback_data="adm_ig"))
        ...
        for k in ["male", "female", "any"]:
            if uid in db["queue"][k]: db["queue"][k].remove(uid)
        save_db(db); bot.edit_message_text("🏡", uid, call.message.id); bot.send_message(uid, "جستجو لغو شد.", reply_markup=main_menu(uid))
