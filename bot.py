...
def handle_messages(message):
    ...
    if uid == OWNER_ID and user and user.get("state") == "waiting_ban_time":
        if message.text and message.text.isdigit():
            ...
            db["users"][uid]["state"] = "main"; save_db(db) # خروج از بن در صورت زدن منو
    ...
    if user["state"] == "reg_name":
        ...
        else: bot.send_message(uid, "❌ **سن نامعتبر!** لطفاً سن خود را به صورت عدد وارد کنید.")
        return
    ...
    if user.get("state") == "in_chat":
        ...
        db["chat_history"][chat_id].append({"u": uid, "val": message.text if message.text else "Media"})
    ...
    # نوشتن پیام ناشناس (رفع باگ تایید/لغو)
...
def callbacks(call):
    ...
    if call.data == "set_m" or call.data == "set_f":
        ...
        if data:
            ...
            try: bot.send_message(data['f'], "🔔 **خبر خوب!** ناشناس پیام شما را مشاهده کرد و هم‌اکنون در حال خواندن آن است.")
            except: pass
        ...
        try: bot.send_message(target, "🚫 حساب شما به دلیل نقض قوانین برای همیشه مسدود شد.")
        except: pass
